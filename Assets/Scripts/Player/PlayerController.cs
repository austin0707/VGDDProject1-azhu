...
public class PlayerController : MonoBehaviour
{
    ...
    // The current move direction of the player. Doesn't include magnitude
    private Vector2 p_Velocity;

    // In order to do anything, we cannot be frozen (timer is 0)
    ...
    private void Awake()
    {
        ...
        for (int i = 0; i < m_Attacks.Length; i++)
        {
            ...
            {
                Debug.LogError(attack.AttackName + " has a windup time that is larger than the amount of time that the player is frozen for");
            }
        }
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (p_Velocity.sqrMagnitude > 0)
        {
            ...
            Vector3 newRot = new Vector3(Mathf.Cos(angleToRotCam) * camForward.x - Mathf.Sin(angleToRotCam) * camForward.z, 0,
                Mathf.Cos(angleToRotCam) * camForward.z + Mathf.Sin(angleToRotCam) * camForward.z);
